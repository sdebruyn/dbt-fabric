...
def _cleanup_lakehouse_entities(
    ...
    """Delete all lakehouse test entities from Purview: process, columns, tables.

    Lakehouse entities have no parent schema entity, so cleanup is simpler than warehouse.
    Uses direct qualified-name lookups (not search) so cleanup works immediately.
    """
    for name in table_names:
        _delete_entity_if_exists(client, "dbt_transformation", f"dbt://model.test.{name}")
...
class TestPurviewSyncLakehouse:
    """Integration test for Purview sync with the Lakehouse code path.

    Verifies that the _create_lakehouse_table() path produces correct entity types,
    qualifiedName format (%252F schema encoding), column types (fabric_lakehouse_table_column
    with camelCase dataType), and that lakehouse types are built-in (no custom registration).
    """
    ...
    def synced_entities(self, project, purview_client, lakehouse_info):
        """Run dbt models targeting Lakehouse and sync to Purview.

        Cleans up stale entities, runs models, syncs, then looks up created entities
        using the %252F qualifiedName format for schema-enabled Lakehouses.
        """
    ...
    def test_qualified_name_format(self, synced_entities, lakehouse_info, project):
        """qualifiedName uses %252F (double-URL-encoded slash) between schema and table."""
        workspace_id, lakehouse_id = lakehouse_info
        schema = project.test_schema
    ...
    def test_column_type_and_data_type_attribute(self, purview_client, synced_entities):
        """Lakehouse columns use fabric_lakehouse_table_column with camelCase dataType."""
    ...
    def test_column_descriptions(self, purview_client, synced_entities):
        """Column descriptions land on userDescription for lakehouse columns."""
        entity = synced_entities["lh_base_model"]
        table_qn = entity["qualifiedName"]
    ...
    def test_no_warehouse_type_registration_needed(self, purview_client):
        """fabric_lakehouse_table and fabric_lakehouse_table_column are built-in Purview types."""
