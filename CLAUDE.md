...
- **Delta Lake DDL has stricter limitations than T-SQL** -- Delta tables in Fabric Lakehouse do not support `DEFAULT` clauses in `ALTER TABLE ADD COLUMN`, `CREATE FUNCTION` (UDFs/UDAFs), or cascading drops (dropping a source table does not automatically drop dependent materialized views). Fix: strip `DEFAULT ...` from column definitions in test helpers, skip tests that require function creation, and skip cascade-dependent tests. When a feature is genuinely unsupported, skip the test with a descriptive reason rather than trying to work around it.

- **Test infrastructure helpers assume T-SQL** -- Several test harness methods (`project.get_tables_in_schema()`, `clear_test_schema`, `project.run_sql()` with raw DDL) use T-SQL system views (`sys.tables`, `sys.views`) or T-SQL DDL syntax that does not exist in Spark SQL. Fix: override the test method to use Spark equivalents (`SHOW TABLES IN`, `SHOW VIEWS IN`, etc.) or skip when no Spark equivalent exists. When a base class assumes inter-test schema cleanup that Spark cannot perform (e.g., drop and recreate schema), split the test class into separate classes with one test each.
