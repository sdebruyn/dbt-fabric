...
class FabricSparkConnection:
    ...
    def __init__(self, livy_session: LivySession | HighConcurrencyLivySession) -> None:
        self._livy_session: LivySession | HighConcurrencyLivySession | None = livy_session
    ...
    def close(self) -> None:
        for cursor in self._cursors:
            cursor.close()
        self._cursors.clear()
        if self._livy_session is not None and hasattr(self._livy_session, "close"):
    ...
    def get_livy_session(self) -> LivySession | HighConcurrencyLivySession:
