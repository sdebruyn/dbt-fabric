{% materialization snapshot, adapter='fabricspark' %}
  ...
  {% else %}
      ...
      {% set staging_table = fabricspark_build_snapshot_staging_table(strategy, sql, target_relation) %}
...
{% macro fabricspark_build_snapshot_staging_table(strategy, sql, target_relation) %}
