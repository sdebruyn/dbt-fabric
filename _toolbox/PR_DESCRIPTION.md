...
The v1.9.10 release ([issue #362](https://github.com/microsoft/dbt-fabric/issues/362), May 2026) adds two "concurrency mitigations" in response to a well-documented user report of `list_<schema>` taking 8–20 minutes under concurrent dbt runs on the same warehouse. The user's diagnosis is precise about what they observed (the metadata read itself hangs while DDL is active in another session) and explicitly flags the rest as hypothesis ("though we have not been able to confirm the exact mechanism"). What landed in v1.9.10 turns that hypothesis into permanent adapter code without further investigation:

1. **`FabricAdapter.list_relations_without_caching` is wrapped in a custom retry-with-exponential-back-off.** This addresses the real complaint (the metadata read hanging), but `dbt-adapters` already provides exactly this on `SQLConnectionManager.add_query` via the `retryable_exceptions` + `retry_limit` arguments. It is not a hidden helper — the same connection manager file already configures it for the project's own `add_query` override, with `retry_limit=3` and `mssql_python.OperationalError` registered. A reviewer reading the diff next to the existing connection manager would notice the duplication.
2. **`FabricConnectionManager.close()` is overridden to suppress a `ROLLBACK`** that the parent issues when `transaction_open=True`. The code comment claims the rollback "can block for minutes on Fabric when concurrent DDL sessions hold catalog locks". The user report does not actually attribute the slowness to the rollback — they only note an unexplained ~11-minute gap between "Close" and "properly closed" and say they have no explanation for it. The "rollback is the culprit" story is the adapter author's own hypothesis layered on top of the user's observation. To check it, we measured `rollback()` directly on a Fabric DW Warehouse under live integration-test load (20 samples, fresh autocommit connections, each after running `SELECT name FROM sys.tables`): every sample completed in ≤ 0.1 ms. The workaround targets a cause that has not been demonstrated.

Both changes share a pattern: real user pain, but the fix is built on an unverified hypothesis and apparently sophisticated workaround code, with no benchmark and no reference to the already-existing dbt-native primitive that solves the same problem. The PR descriptions follow the same AI-generated patterns observed in PR #315. The cumulative effect is an adapter that accretes defensive code without evidence — the opposite of the lean, measured posture a production-grade dbt adapter needs.
...
To be attached as separate files alongside this PR description at submission time:

1. **Concrete bug-fix track record.** A table listing 5–10 bugs that this fork has fixed and that remain active in `microsoft/dbt-fabric` and/or `microsoft/dbt-fabricspark`. Per bug: short description, link to the fix PR/commit in this codebase, status check in upstream (open issue / closed without fix / not yet reported). Source: `git log` of the fork combined with `gh search issues` against the upstream repos.
2. **Customers in production.** A list of organizations (with their consent) currently running this adapter in production, with optional quote fragments about their migration reasons. Where public attribution is not possible, anonymized by sector and size.
3. **Conference talks and community validation.** Links to talks, blog posts, and podcast mentions where the adapter has been discussed. Reinforces that this is not a one-person hobby but part of a wider community ecosystem.
4. **Live demo / video** of the adapter in action — particularly the dbt-native features (warehouse snapshots in hooks, external tables via sources, Purview sync via `on-run-end`).
5. **Test class catalog** — clickable index of every integration test class in `tests/`.
6. **Architectural decision records** for the largest design choices: instance encapsulation, dbt-spark inheritance, dispatch-based package overrides, unified token provider, deterministic Livy session tagging.
7. **CONTRIBUTING.md "Upgrading dbt-core support" section** — direct link to demonstrate the systematic upgrade discipline.
