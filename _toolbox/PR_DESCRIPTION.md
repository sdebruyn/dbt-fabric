...
(The same release also adds a `close()` override that skips `ROLLBACK` on autocommit connections, attributed to pyodbc-level contention. I haven't measured pyodbc, so I'm not calling that part wrong. On `mssql-python` I measured `rollback()` at ≤0.1 ms under load, so there's nothing for me to port there.)
