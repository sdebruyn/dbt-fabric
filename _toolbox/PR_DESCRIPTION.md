...
A second symptom that the list does not capture by itself: thin CI matrices. `microsoft/dbt-fabric` runs unit tests on Python 3.10 and 3.11 and integration tests only on 3.11; `microsoft/dbt-fabricspark` runs everything in a single Docker container with no Python matrix at all. The PyPI classifier list on `microsoft/dbt-fabric` advertises Python 3.8–3.12; 3.13 has been GA for over a year and is still missing. Python-version regressions in a dbt adapter are genuinely rare so this is not catastrophic, but anyone running on Python 3.13 has no validation that the adapter actually works on their interpreter — and the small CI footprint is what lets the per-PR drift documented above accumulate.
