...
**Warehouse snapshots wired into the connection lifecycle.** The warehouse-snapshot feature in `microsoft/dbt-fabric` is implemented as hooks inside `open()` of the connection manager, plus `atexit` handlers. That couples a user-facing feature to Python runtime internals and to connection-manager lifecycle methods that aren't designed as hook points. Not part of dbt's stable adapter interface. *User impact:* snapshots can only happen at fixed lifecycle points the adapter decides. No dynamic snapshot names (e.g. dated by Jinja). No per-model snapshots via `post-hook`. No triggering from `dbt run-operation`. And the `atexit` failure mode applies — a snapshot can silently not happen with no error in dbt's output. This contribution exposes warehouse snapshots as a Jinja macro you can call from `on-run-start` / `on-run-end`, which is the documented, supported way to do this in dbt.
...
**A pattern.** Looking across these items: PR #315 ships a parameter that does nothing. The v1.9.10 retry wrapper adds an adapter-method retry layer where `add_query`'s built-in `retryable_exceptions` was the right hook. The v1.9.10 `delete_condition` config invents adapter-private knobs on a dbt-core materialization. The v1.10.0 atexit warehouse snapshots reinvent `on-run-end`. Each of these is code that *looks* sophisticated but ignores a dbt-native primitive that already does the job. Combined with PR descriptions that read as AI-generated, the symptom is clear: AI-assisted code is landing without review from someone with the dbt expertise to spot that the wheel was already there.
