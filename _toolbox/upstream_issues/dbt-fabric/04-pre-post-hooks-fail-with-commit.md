...
```shell
git ls-tree upstream/main dbt/include/fabric/macros/materializations/
# (returns models/, snapshots/, seeds/, snapshot/, but no hooks.sql)
```

No `fabric__run_hooks` override exists. dbt-adapters' default — at `dbt/include/global_project/macros/materializations/hooks.sql` — calls `commit;` between hook batches.
