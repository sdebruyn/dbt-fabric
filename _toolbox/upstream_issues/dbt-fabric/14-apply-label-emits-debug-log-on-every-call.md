...
1. The `apply_label` macro in `dbt/include/fabric/macros/adapters/metadata.sql` opens with `{{ log(config.get('query_tag','dbt-fabric')) }}` — a debug leftover that fires on every macro invocation.
2. The `apply_label()` helper is called by hand from at least seven different macros. That is the wrong shape for the job: dbt-adapters ships a generic per-query header/footer mechanism (`MacroQueryStringSetter` + `set_query_header`) that every other reference adapter uses. Threading a custom helper through every materialization and metadata macro both invites the bug above and creates incidental coupling — e.g. the custom `fabric__snapshot_merge_sql` (separate issue) exists in part because it has to thread `apply_label()` between its UPDATE and INSERT statements.
...
- If query-label tagging is needed for observability, the right home is the connection-manager `query_header`, not a per-statement Jinja helper.
