...
The PR diff shows the parameter being threaded through every `get_token()` invocation, but `FabricCredentials` (defined in `dbt/adapters/fabric/fabric_credentials.py`) does not declare a `login_timeout` field, and the `azure-identity` SDK's `TokenCredential.get_token` signature does not accept a `timeout` keyword.
